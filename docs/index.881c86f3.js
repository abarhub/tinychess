...
(function() {
    ...
    function validSemanticVersion(version, minimum) {
        ...
        var versionNum = version.major * 10000000000 + version.minor * 100000 + version.patch;
        var minimumNum = minimum.major * 10000000000 + minimum.minor * 100000 + minimum.patch;
    ...
    function interpolateTemplate(str, obj) {
        for(var key in obj){
            ...
            var value = obj[key];
            while(str.indexOf(keyTemplateStr) !== -1)str = str.replace(keyTemplateStr, value);
        }
        return str;
    }
    if (RUN_ASSERTS) {
    ...
    function validFen(fen) {
        ...
        // check each section
        for(var i = 0; i < 8; i++){
            if (chunks[i].length !== 8 || chunks[i].search(/[^kqrnbpKQRNBP1]/) !== -1) return false;
        }
        return true;
    }
    if (RUN_ASSERTS) {
    ...
    function pieceCodeToFen(piece) {
        ...
        // white piece
        if (pieceCodeLetters[0] === "w") return pieceCodeLetters[1].toUpperCase();
        // black piece
    ...
    function fenToObj(fen) {
        ...
        for(var i = 0; i < 8; i++){
            ...
            for(var j = 0; j < row.length; j++)// number / empty squares
    ...
    function objToFen(obj) {
        ...
        for(var i = 0; i < 8; i++){
            for(var j = 0; j < 8; j++){
                ...
                // piece exists
                if (obj.hasOwnProperty(square)) fen = fen + pieceCodeToFen(obj[square]);
                else // empty space
                fen = fen + "1";
            }
            if (i !== 7) fen = fen + "/";
    ...
    function findClosestPiece(position, piece, square) {
        ...
        for(var i = 0; i < closestSquares.length; i++){
            ...
            if (position.hasOwnProperty(s) && position[s] === piece) return s;
        }
        return false;
    }
    ...
    function createRadius(square) {
        ...
        for(i = 0; i < squares.length; i++)surroundingSquares.push(squares[i].square);
    ...
    function buildContainerHTML(hasSparePieces) {
        ...
        if (hasSparePieces) html += '<div class="{sparePieces} {sparePiecesTop}"></div>';
        html += '<div class="{board}"></div>';
        if (hasSparePieces) html += '<div class="{sparePieces} {sparePiecesBottom}"></div>';
        html += "</div>";
    ...
    function expandConfigArgumentShorthand(config) {
        ...
        else if (validFen(config)) config = {
            position: fenToObj(config)
        };
        else if (validPositionObject(config)) config = {
    ...
    function expandConfig(config) {
        ...
        if (!config.hasOwnProperty("pieceTheme") || !isString(config.pieceTheme) && !isFunction(config.pieceTheme)) config.pieceTheme = "img/chesspieces/wikipedia/{piece}.png";
        // animation speeds
    ...
    function checkContainerArg(containerElOrString) {
        ...
        if (isString(containerElOrString) && containerElOrString.charAt(0) !== "#") containerElOrString = "#" + containerElOrString;
    ...
    function constructor(containerElOrString, config) {
        ...
        function error(code, msg, obj) {
            ...
            if (config.hasOwnProperty("showErrors") !== true || config.showErrors === false) return;
            ...
            // custom function
            if (isFunction(config.showErrors)) config.showErrors(code, msg, obj);
        }
        function setInitialState() {
            currentOrientation = config.orientation;
            // make sure position is valid
            if (config.hasOwnProperty("position")) {
                if (config.position === "start") currentPosition = deepCopy(START_POSITION);
                else if (validFen(config.position)) currentPosition = fenToObj(config.position);
                else if (validPositionObject(config.position)) currentPosition = deepCopy(config.position);
                else error(7263, "Invalid value passed to config.position.", config.position);
            }
        }
        // -------------------------------------------------------------------------
        // DOM Misc
        ...
        function calculateSquareSize() {
            ...
            while(boardWidth % 8 !== 0 && boardWidth > 0)boardWidth = boardWidth - 1;
            return boardWidth / 8;
        }
        // create random IDs for elements
        function createElIds() {
            // squares on the board
            for(var i = 0; i < COLUMNS.length; i++)for(var j = 1; j <= 8; j++){
            ...
            for(var i = 0; i < 8; i++){
                ...
                for(var j = 0; j < 8; j++){
                    ...
                    if (config.showNotation) {
                        // alpha notation
                        if (orientation === "white" && row === 1 || orientation === "black" && row === 8) html += '<div class="{notation} {alpha}">' + alpha[j] + "</div>";
                        // numeric notation
                        if (j === 0) html += '<div class="{notation} {numeric}">' + row + "</div>";
                    }
                    html += "</div>" // end .square
                    ;
                ...
                squareColor = squareColor === "white" ? "black" : "white";
                if (orientation === "white") row = row - 1;
                else row = row + 1;
            }
            return interpolateTemplate(html, CSS);
        }
        function buildPieceImgSrc(piece) {
            ...
            if (isString(config.pieceTheme)) return interpolateTemplate(config.pieceTheme, {
                piece: piece
            });
        ...
        function buildSparePiecesHTML(color) {
            ...
            var html = "";
            for(var i = 0; i < pieces.length; i++)html += buildPieceHTML(pieces[i], false, sparePiecesElsIds[pieces[i]]);
            return html;
        }
        // -------------------------------------------------------------------------
        // Animations
        ...
        function doAnimations(animations, oldPos, newPos) {
            ...
            for(var i = 0; i < animations.length; i++){
                ...
                // clear a piece
                if (animation.type === "clear") $("#" + squareElsIds[animation.square] + " ." + CSS.piece).fadeOut(config.trashSpeed, onFinishAnimation3);
                else if (animation.type === "add" && !config.sparePieces) $("#" + squareElsIds[animation.square]).append(buildPieceHTML(animation.piece, true)).find("." + CSS.piece).fadeIn(config.appearSpeed, onFinishAnimation3);
                else if (animation.type === "add" && config.sparePieces) animateSparePieceToSquare(animation.piece, animation.square, onFinishAnimation3);
                else if (animation.type === "move") animateSquareToSquare(animation.source, animation.destination, animation.piece, onFinishAnimation3);
            }
        }
        ...
        function isXYOnSquare(x, y) {
            for(var i in squareElsOffsets){
                ...
                if (x >= s.left && x < s.left + squareSize && y >= s.top && y < s.top + squareSize) return i;
            }
            return "offboard";
        }
        ...
        function snapbackDraggedPiece() {
            ...
            function complete() {
                ...
                if (isFunction(config.onSnapbackEnd)) config.onSnapbackEnd(draggedPiece, draggedPieceSource, deepCopy(currentPosition), currentOrientation);
            }
        ...
        function beginDraggingPiece(source, piece, x, y) {
            ...
            if (isFunction(config.onDragStart) && config.onDragStart(source, piece, deepCopy(currentPosition), currentOrientation) === false) return;
            // set state
            isDragging = true;
            draggedPiece = piece;
            draggedPieceSource = source;
            // if the piece came from spare pieces, location is offboard
            if (source === "spare") draggedPieceLocation = "offboard";
            else draggedPieceLocation = source;
            ...
            if (source !== "spare") // highlight the source square and hide the piece
        ...
        function updateDraggedPiece(x, y) {
            ...
            if (validSquare(draggedPieceLocation)) $("#" + squareElsIds[draggedPieceLocation]).removeClass(CSS.highlight2);
            // add highlight to new square
            if (validSquare(location)) $("#" + squareElsIds[location]).addClass(CSS.highlight2);
            // run onDragMove
            if (isFunction(config.onDragMove)) config.onDragMove(location, draggedPieceLocation, draggedPieceSource, draggedPiece, deepCopy(currentPosition), currentOrientation);
            // update state
        ...
        function stopDraggedPiece(location) {
            ...
            if (isFunction(config.onDrop)) {
                ...
                if (draggedPieceSource === "spare" && validSquare(location)) // add the piece to the board
                newPosition[location] = draggedPiece;
                // source piece was on the board and position is off the board
                if (validSquare(draggedPieceSource) && location === "offboard") // remove the piece from the board
                delete newPosition[draggedPieceSource];
        ...
        widget.resize = function() {
            ...
            // spare pieces
            if (config.sparePieces) $container.find("." + CSS.sparePieces).css("paddingLeft", squareSize + boardBorderSize + "px");
            // redraw the board
            drawBoard();
        };
 ...
 * environment */ if (typeof exports !== "undefined") exports.Chessboard = window.Chessboard;

//# sourceMappingURL=index.881c86f3.js.map
